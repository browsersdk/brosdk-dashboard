...
export function EnvironmentBatchBar({
  ...
  return (
    <div className="environment-batch-bar" role="toolbar" aria-label="批量环境操作">
      ...
      <div>
        <button className="button secondary compact" type="button" disabled={!desktop || busy || startIds.length === 0} onClick={() => onAction("start", startIds)}>
          {busy ? <LoaderCircle className="spin" size={14} /> : <Play size={14} />}启动 {startIds.length}
        </button>
        <button className="button secondary compact" type="button" disabled={!desktop || busy || stopIds.length === 0} onClick={() => onAction("stop", stopIds)}>
          <Square size={14} />停止 {stopIds.length}
        </button>
        <button className="icon-button" type="button" title="清除选择" aria-label="清除选择" disabled={busy} onClick={onClear}><X size={15} /></button>
      </div>
    </div>
  );
}
