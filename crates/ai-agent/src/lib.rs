...
impl AiClient {
    ...
    pub async fn plan(
        ...
        let context_text = serde_json::to_string(context).unwrap_or_else(|_| "{}".into());
        let system = format!(
            "You plan one controlled BroSDK Dashboard action. Return JSON only with keys: summary, action, envId, arguments. Allowed action values: none, environment.start, environment.stop, environment.sync, runtime.reconcile, proxy.diagnose, environment.diagnose, mcp.read, mcp.call. mcp.read is the compatibility action for bounded reads. mcp.call invokes one DLL-advertised tool and requires arguments shaped as {{\"tool\":\"tool_name\",\"arguments\":{{...}}}}. Set envId for a single-environment browser tool; omit envId only for a global management read. The global MCP catalog names browser tools with the env. prefix, for example env.tabs, env.snapshot, and env.act. Do not put envId inside arguments because the Manager injects the selected envId. Current browser tool base names include browser_state, tabs, bookmarks, history, tab_groups, navigate, snapshot, diff, act, download, upload, read, grep, screenshot, pdf, wait, windows, and evaluate, but runtime tools/list is authoritative. envId is required for environment actions and single-environment MCP. The Manager resolves the final target, expected state, and idempotency key from current local state. Never say accepted means ready.\n\nCurrent Dashboard snapshot:\n{context_text}"
        );
