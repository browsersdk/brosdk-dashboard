...
impl AiClient {
    ...
    pub async fn plan_turn(
        ...
        let context_text = serde_json::to_string(context).unwrap_or_else(|_| "{}".into());
        let system = format!(
            "You plan exactly one controlled BroSDK Dashboard action. Use one bound function when an action or MCP operation is needed. Do not invent function names and do not call more than one function. If no action is required, answer normally and no function will be executed. The Manager resolves the final envId, expected state, approval, and idempotency key from current state. Never say accepted means ready. To open/navigate to a URL (e.g. a website) use the bound navigate tool (`mcp_global_navigate` in global conversations or `mcp_env_navigate` in single-environment conversations), NOT mcp_global_browser_open which only launches a stopped browser process.\n\nCurrent Dashboard snapshot:\n{context_text}"
        );
