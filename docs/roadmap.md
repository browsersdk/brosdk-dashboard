...
| 34. 全界面动作状态收口 | P1 | 已完成 | 禁用动作原因、稳定图标按钮尺寸和逐页 E2E 守护 |
| 35. 跨境店铺与环境绑定 | P1 | 规划中 | 店铺账号绑定 envId，建立 Commerce 基础对象与打开后台入口 |
| 36. 订单与发货工作台 | P1 | 规划中 | 多平台订单同步、未发货筛选、物流单号回传和失败重试 |
| 37. SKU 与库存同步 | P1 | 规划中 | 统一 SKU、平台 listing 映射、库存与价格同步 |
| 38. 商品资料与刊登同步 | P2 | 规划中 | 商品资料、平台刊登草稿、预检和高风险字段批准 |
| 39. Commerce Agent | P2 | 规划中 | 受控 AI 处理订单、发货、SKU 同步和店铺后台辅助 |
...
目标：把逐页审查中发现的“按钮灰掉但不知道原因”和小尺寸图标按钮问题产品化收口，让多环境日常操作失败前就可理解、可恢复。

实现结果（2026-07-27）：

- 新增共享 `actionTitle` 与 `desktopActionReason`，总览、环境、指纹、代理、内核、MCP、AI、操作和设置页面的禁用按钮统一提供悬停原因。
- 环境、批量操作、详情、创建、代理、指纹、内核、设置、API Key 初始化和 AI Provider 等关键动作区分桌面预览、忙碌、未选环境、环境未 ready、缺少 URL、缺少内核和凭据缺失等不可执行原因。
- 表格动作列按双图标按钮重新留宽，基础 `.icon-button` 固定最小宽高和不可压缩尺寸；AI 会话删除按钮保持稳定 32px 命中区。
- Playwright 新增桌面/移动逐页守护，断言可见禁用按钮必须有 `title`，可见图标按钮宽高不小于 30px，并继续执行健康布局检查。
- 浏览器插件重新覆盖 9 个主要页面，结果为无横向溢出、无缺失禁用原因、无小按钮、无文本裁切；关键页面截图保存在本次 QA 临时目录。
- 验证通过 Dashboard 56 项组件测试、Playwright 桌面/移动 26 项、`npm run check`、Rustfmt 和 Clippy。
...
- 点击内核“安装或更新”后，Dashboard 不等待 SDK 返回就显示顶部进度面板和对应行内状态，文案为“已发送安装请求，等待 SDK 受理”，状态列主徽标显示“安装中”，按钮保持禁用并显示旋转图标。
- Manager 将 `kernel.install` 的非终态 SDK/DLL 回调写入 operation message；`browser-install-success` 与失败事件继续切换终态，不把原始 payload 暴露给 Dashboard。收到安装成功回调后，Manager 会后台重扫 `sdk_info`、init catalog、服务端 `kernelList` 和本地 cores，让安装完成后的 `kernel_records` 尽快从“可安装”刷新为本地已安装状态。
...
- 组件测试覆盖真实桌面点击后 `installKernel` Promise 未返回时的即时反馈、下载进度、成功终态和跨平台同版本 catalog 过滤；Playwright 桌面/移动覆盖 `browser-install · Downloading · 42%` 的顶部/行内可见状态、状态列“安装中”、当前平台过滤、安装按钮禁用、无横向溢出和控制台健康。
...
- 新增 Commerce 基础模型：store、auth、product、sku、listing、order、fulfillment 和 sync job。
- 店铺以 `storeId` 为本地主键，绑定一个默认 `envId`；店铺账号和环境身份必须可辨识、可搜索、可审计。
- 店铺详情提供平台、店铺名、授权状态、默认仓/物流策略、备注和一键打开绑定环境。
- 店铺后台打开继续走 Manager operation 和 DLL MCP，不把后台凭据、Cookie、平台 token 暴露给 AI prompt 或日志。
- 详细边界见 [commerce-roadmap.md](commerce-roadmap.md)。

验收：

- 店铺绑定不会改变现有环境事实源；环境仍以 SDK 服务端 `envId` 为唯一主键。
- 未绑定环境的店铺不能执行浏览器后台动作，UI 必须给出可恢复原因。
- 无真实平台授权时可用 mock connector 完成 UI、operation 和缓存流转测试。

## 39. 阶段 36：订单与发货工作台

目标：优先解决跨境卖家的高频流程：拉取订单、筛选未发货、录入物流并回传平台。

规划任务：

- 新增订单列表、订单详情、店铺筛选、平台筛选、未发货视图和异常同步状态。
- 订单同步通过 connector 进入 Manager operation；本地只保存带同步时间、新鲜度和失败原因的缓存。
- 发货回传必须记录物流商、单号、目标平台订单 id、脱敏请求摘要和平台响应摘要。
- 首个真实 connector 只要求只读拉单；写操作可先用 mock connector 与 UI/operation 验收。

验收：

- 买家手机号、完整地址、邮箱和平台 token 不进入日志、截图、operation 明文和 AI prompt。
- 回传失败可重试；重试不重复创建不可控的发货记录。
- 平台 API 缺口用绑定环境打开后台补位，不伪装成已完成自动化。

## 40. 阶段 37：SKU 与库存同步

目标：建立统一 SKU 与平台 listing 映射，先覆盖库存和价格这两个最小同步闭环。

规划任务：

- 新增 SKU 中心、平台 listing 映射、库存策略、价格策略和同步状态。
- connector 提供 `syncInventory`、`syncPrice` 和 `refreshListing`。
- 平台私有字段留在 connector adapter 中，通用 UI 只展示稳定字段和脱敏异常。

验收：

- SKU、listing 和平台外部 id 必须唯一且可追踪。
- 库存/价格写操作默认需要批准，自动模式只允许显式白名单。
- 同步前后有 operation 记录和平台响应摘要。

## 41. 阶段 38：商品资料与刊登同步

目标：在订单和库存稳定后，再接入复杂度更高的商品资料和平台刊登。

规划任务：

- 新增 SPU/SKU 商品资料、图片引用、标题、属性模板和平台刊登草稿。
- connector 提供刊登预检、提交、状态刷新和失败报告。
- 高风险字段如标题、品牌、类目、合规属性和图片替换必须人工批准。

验收：

- 商品资料本地是运营草稿，不覆盖平台事实；平台刊登状态以平台 API 返回为准。
- 刊登失败必须能定位到平台、店铺、listing、字段和错误摘要。
- 不把图片原始路径、上传 token 或平台凭据写入日志。

## 42. 阶段 39：Commerce Agent

目标：让 AI 在受控 commerce tools 内处理日常运营问题，延续当前 Chat/Agent 的审批和 operation 体系。

规划任务：

- 新增 `commerce.store.*`、`commerce.order.*`、`commerce.fulfillment.*`、`commerce.sku.*`、`commerce.listing.*` 工具目录。
- Chat 默认只读；Agent 写操作默认逐次批准；自动执行只允许低风险读取和明确白名单写操作。
- Agent 步骤展示店铺、平台、envId、工具名、operation 和脱敏参数。
- 支持“检查今天未发货订单”“同步某 SKU 库存”“打开店铺后台查看异常订单”等意图。

验收：

- Agent 不能直接持有平台 token、店铺后台密码、API Key、userSig、完整 CDP URL 或买家隐私。
- 每个写操作都必须关联 storeId、外部平台 id、operation 和审批/自动执行来源。
- 单环境会话不能越过创建时绑定的 envId；全局会话操作多个店铺时必须显式展示目标店铺和环境。
