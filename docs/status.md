...
1. AI 会话默认仅保存在当前 WebView 内存；用户显式开启“保存历史”后仍暂存 WebView localStorage，正式公开发布前需要迁入 Manager 受保护存储或继续作为明确的可关闭选项。
...
6. 项目源码已采用 MIT License；仓库包含的 brosdk.dll 和 brosdk.h 仍需在公开分发前确认 DLL 再分发条款。
