...
- 多环境、代理、内核、指纹、操作、设置和 AI 助手这些领域能力。
- 白色扁平视觉风格和最小窗口约束。

当前 Dashboard 已把开发期页面清单收敛为“工作台、环境、自动化、资源、系统”五个一级入口。旧客户端或历史文档中的总览、环境、指纹、代理、内核、操作、设置、AI 助手等入口，在本项目中作为组内页面保留稳定深链，而不是继续暴露为九个一级导航按钮。
