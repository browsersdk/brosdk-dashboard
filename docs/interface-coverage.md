...
审计日期：2026-07-29。审计依据：本地接口快照、browser-open-server 的 browser router、仓库 `libs/windows_x64/brosdk.h`、DLL 源码及参考客户端实现。接口快照和外部源码路径不属于本仓库公开文档依赖。
...
| `kernelList` | Manager 使用受保护 API Key 调 `/api/v2/browser/kernelList`（`page/pageSize/status=1` + 当前 `platform/arch`），并与 `sdk_init.data.kernelVersions`、`sdk_info` catalog、本地 cores 合并；Dashboard 不直连 | 已产品化（Manager/API Key + DLL/Host 链路） |
...
- 会话默认只保存在当前 WebView 进程内存；用户显式开启“保存历史”后才写入 WebView 本地存储。持久化时最多 20 个会话、每个 80 条消息；请求只携带最近 40 条，Manager 再限制单条 16 KiB、总计 128 KiB。
- 支持新建、切换、清空和删除；Dashboard 不主动注入 API Key、userSig、完整 CDP URL 或 SDK 原始响应。用户手动输入的文本会原样进入本地会话，因此不应在对话框粘贴凭据。
- 会话创建时固定为全局或单环境。全局会话可从用户文本中的单个已同步 envId 解析本次目标；单环境会话始终使用创建时绑定的 envId，文本中的其它环境必须被拒绝。Manager 在计划返回 UI 前写入真实 `expectedState` 和 UUID 幂等键。
...
会话历史默认不持久化、不跨设备同步，也不进入 Manager SQLite。显式开启“保存历史”后，它属于可清除的本机 UI 数据，不是环境数据缓存；清空或删除会话会立即更新本地存储。正式公开发布前应迁入 Manager 受保护存储，或继续作为明确可关闭的非默认选项。

## 后续顺序

1. P0：AI 持久化历史迁入受保护存储，以及上传、脚本执行、下载和页面写操作的风险分级与逐次批准。
2. P0：`sdk_token_update` 在线 userSig 刷新与失败回退到 Host 重建。
3. P1：Cookie 查看、导入、导出及敏感字段/文件边界。
4. P1：本地扩展扫描、manifest 校验和按环境启动选择。
5. P2：专家指纹选项与全局指纹模板，不改变普通创建的两字段流程。
