...
会话历史目前不加密、不跨设备同步，也不进入 Manager SQLite。它属于可清除的本机 UI 数据，不是环境数据缓存；清空或删除会话会立即更新本地存储。正式公开发布前应迁入 Manager 受保护存储或提供默认不持久化模式。

## 后续顺序

1. P0：AI 会话受保护存储，以及上传、脚本执行、下载和页面写操作的风险分级与逐次批准。
